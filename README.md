...
Impact vs. Class: The distribution shows that MODERATE impact variants are the most prevalent across both classes. While class 0 (benign/reference) variants are more frequent than class 1 (pathogenic) across all impact categories, the relative proportion of class 1 variants remains consistent, suggesting that impact is a significant descriptive feature but not a standalone classifier.
Origin vs. Class: The vast majority of variants fall into a single origin category, creating a heavily skewed distribution. Because this feature lacks variance (it is effectively a constant), it holds minimal predictive power and is a prime candidate for removal to simplify the model.
Strand vs. Class: The distribution is strikingly uniform across both -1.0 and 1.0 strands for both classes. This symmetry confirms that the dataset is well-balanced regarding genomic orientation, ensuring that the model will not inherit strand-specific biases.
Biotype vs. Class: The data is almost exclusively composed of protein_coding variants. Similar to origin, the lack of variety in this feature suggests it may contribute little information to the model's decision-making process.
Consequence vs. Class: This feature displays the highest level of informative variance. missense_variant and synonymous_variant are the most frequent consequences, and the clear differences in counts between class 0 and class 1 indicate that this feature will likely be a primary driver for the model’s ability to distinguish between variant types.
